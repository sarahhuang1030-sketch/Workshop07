...
export default function EmployeeChatWorkspace({ role = "agent" }) {
    ...
    useEffect(() => {
        ...
        if (
            ...
            payload?.requestId &&
            Number(payload.requestId) === Number(selectedRequest.requestId)
        ) {
            setSelectedRequest((prev) => {
                if (!prev) return prev;

                const nextStatus = String(payload?.status || "CLOSED").toUpperCase();
                const prevStatus = String(prev.status || "").toUpperCase();

                if (
                    prevStatus === nextStatus &&
                    prev.closedAt === (payload?.closedAt || prev.closedAt) &&
                    Number(prev.conversationId) === Number(payload?.conversationId ?? prev.conversationId)
                ) {
                    return prev;
                }
                ...
                return {
                    ...
                    assignedEmployeeUserId:
                        payload?.assignedEmployeeUserId ?? prev.assignedEmployeeUserId
                };
            });

            setSelectedConversation((prev) => {
                if (!prev) return prev;
                if (Number(prev.conversationId) !== Number(payload?.conversationId)) return prev;

                const prevStatus = String(prev.status || "").toUpperCase();
                if (prevStatus === "CLOSED") return prev;
    ...
    const loadRequests = async () => {
        ...
        try {
            ...
            if (selectedRequest?.requestId) {
                const updatedRequest = safeData.find(
                    (r) => Number(r.requestId) === Number(selectedRequest.requestId)
                );
    ...
    const handleCancelRequest = async (req) => {
        ...
        try {
            const res = await apiFetch(
                `/api/chat/chat-requests/${req.requestId}/cancel-by-employee?employeeUserId=${currentUserId}`,
                { method: "POST" }
            );

            if (!res.ok) {
                const errorText = await res.text();
                console.error("Cancel request failed:", res.status, errorText);
                return;
            }
    ...
    const conversationsForSelectedCustomer = useMemo(() => {
        ...
        return [...customerConversationHistory].sort((a, b) => {
