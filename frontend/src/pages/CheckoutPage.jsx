...
export default function CheckoutPage() {
    ...
    const pricing = useMemo(() => {
        ...
        if (externalInvoice) {
            ...
            const waterBill = subtotal * 0.05;
            const tax = (subtotal + waterBill) * taxRate;

            return {
                subtotal,
                waterBill,
                tax,
                finalTotal: subtotal + waterBill + tax,
        ...
        if (billingCycle === "yearly" && hasRecurring) {
            calculatedSubtotal = (baseSubtotal - recurringMonthly) + (yearlyBase - yearlyDiscount);
        }

        const waterBill = calculatedSubtotal * 0.05;
        const tax = (calculatedSubtotal + waterBill) * taxRate;
        const finalTotal = calculatedSubtotal + waterBill + tax;
    ...
    return (
        <Container style={{ maxWidth: 700 }} className="py-5">
            ...
            <Card className="mt-3 p-3">
                ...
                <div className="d-flex justify-content-between">
                    <span>Water Bill Fee (5%)</span>
                    <span>${(pricing.waterBill ?? 0).toFixed(2)}</span>
                </div>
