...
export default function ShoppingCartPage() {
    // addAddOn is needed so user can pick add-ons from cart page
    const { plan, addOns, addAddOn, removeAddOn, removePlan } = useCart();
    const navigate = useNavigate();

    //  NEW: load add-ons for selection on cart
    const [allAddOns, setAllAddOns] = useState([]);
    const [loadingAddOns, setLoadingAddOns] = useState(false);
    const [addOnsError, setAddOnsError] = useState("");

    // Determine selected plan type (Mobile vs Internet)
    // Prefer serviceTypeId if your plan has it; fallback to name/serviceType text.
    ...
    // Fetch add-ons ONCE a plan exists
    useEffect(() => {
        let cancelled = false;
    ...
    // Check whether backend is returning serviceTypeId for add-ons
    const addOnsHaveServiceTypeId = useMemo(() => {
        if (!allAddOns || allAddOns.length === 0) return false;
        const sample = allAddOns[0];
        return (
            sample?.serviceTypeId != null ||
            sample?.ServiceTypeId != null ||
            sample?.serviceTypeID != null ||
            sample?.ServiceType != null
        );
    }, [allAddOns]);
    ...
    return (
        ...
        <div className="py-5" style={{ background: "#f8fafc", minHeight: "100vh" }}>
            <Container style={{ maxWidth: 900 }}>
                ...
                {/* AVAILABLE ADD-ONS TO SELECT (matches plan when serviceTypeId exists) */}
                <Card className="mb-4 shadow-sm border-0" style={{ borderRadius: 18 }}>
                    <Card.Body className="p-4">
                        ...
                        {!loadingAddOns && addOnsError && <Alert variant="danger">{addOnsError}</Alert>}
                                        ...
                                        <Col key={a.addOnId} md={6}>
                                            <Card className="h-100">
                                                <Card.Body className="d-flex flex-column">
                                                    ...
                                                    {a.description && (
                                                        <div className="text-muted small mb-3">{a.description}</div>
                                                    )}
                                                    ...
                                                    >
                                                        {alreadyAdded ? <Trash2 size={16} /> : <Plus size={16} />}
